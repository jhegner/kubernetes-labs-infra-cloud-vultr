...
PASSPHRASE="minha_frase_secreta_super_segura_win" # <-- EDITE AQUI SUA FRASE SECRETA

# 2. Caminho completo e nome do arquivo da chave (sem a extensão .pub)
#    No Windows, o HOME geralmente aponta para /c/Users/SeuUsuario no Git Bash/WSL.
#    Exemplo: ~/.ssh/minha_chave_vultr_win
KEY_PATH_NAME="$HOME/.ssh/id_rsa_vultr_win" # <-- EDITE AQUI O CAMINHO/NOME DA CHAVE

# 3. Comentário para sua chave pública
#    Útil para identificar a chave depois (ex: seu e-mail, nome do projeto/servidor)
KEY_COMMENT="minha_chave_vultr_win@meu-windows" # <-- EDITE AQUI O COMENTÁRIO

# --- FIM DAS CONFIGURAÇÕES ---

echo "--- Gerador de Chaves SSH para Windows (Git Bash/WSL) ---"
echo "Configurações:"
...
# O diretório .ssh no Windows (via Git Bash/WSL) mapeia para C:\Users\SeuUsuario\.ssh
if [ ! -d "$HOME/.ssh" ]; then
